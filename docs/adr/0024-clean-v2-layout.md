...
- **Stage 1（已完成）**：core(契约定稿) · llm(原样保留) · tool(重写契约) · bus · checkpoint ·
  ...
  quickstart。后经 ADR 0025 把 runtime 合并进 agent(Functional Options + New/Run/Stream/Resume)。
- **Stage 2（已完成）**：workflow agent（Sequential/Parallel/Loop/Pipeline）+ transfer（LLM 驱动委派,
  自动 transfer_to_agent 工具）+ subagent 上下文隔离（AsTool）+ WithOutputKey 阶段间传值;
  统一走 sealed `Runnable`(run→outcome);demo:workflow/multiagent/subagent/pipeline/refine。
- **Stage 3（已完成）**：concrete `middleware` 包实现 `agent.Middleware`：Tracing / RateLimit /
  Permission(allow/ask-HITL/deny) / Compaction / RAG(含内存检索器),外加 RetryModel(模型装饰器)。
  demo:examples/middleware。配套修正:BeforeTool 门禁现支持 Stop/Escalate/Transfer(此前仅 Interrupt)。
- **Stage 4(基本完成)**:已从 `v0.0.1` 移植并适配:embeddings、sandbox + tool/exec、
  memory 向量库(作 `middleware.RAG` 的 Retriever)、checkpoint 文件后端(跨进程持久恢复)、
  prompt builder(`WithPrompt`)、skill(文件系统技能包,三层渐进加载)、分层记忆系统
  (workingmem / textmem / projectmem / rules / memx)、queue(背景执行:队列 + 工作池,
  `EnqueueAgent` 桥接;含 MemQueue 内存后端与 Redis Streams 跨进程后端 + Redis Pub/Sub 进度总线)。**未移植**:session 线程索引
  ——v2 由 checkpoint 覆盖状态/恢复,thread 概念已内建于 `OnThread` + checkpointer,无需独立包。
- **Stage 5**:补齐 demo（streaming-bus / branch / deepagent / media）与 README、
  ARCHITECTURE 文档。
...
- **包数更少、依赖更直、无桥接代码**：core 吸收 Event、tool 原生带 Control/State、执行模型
  收进 `agent` 一包、runner→runtime、scheduler→queue、session 退化。
