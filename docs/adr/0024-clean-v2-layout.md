...
- **Stage 4(进行中)**:已从 `v0.0.1` 移植并适配:embeddings、sandbox + tool/exec、
  memory 向量库(作 `middleware.RAG` 的 Retriever)、checkpoint 文件后端(跨进程持久恢复)、
  prompt builder(`WithPrompt`)、skill(文件系统技能包,三层渐进加载)、**分层记忆系统**
  (workingmem 工作记忆穿越压缩 / textmem 文本记忆 / projectmem 项目记忆 AGENTS.md /
  rules 两级规则 / memx 装配 + 固化,适配 core.State / Result.State / `[]agent.Middleware`)。
  **待移植**:scheduler(背景执行,bus+Run 已可用 goroutine 替代)、session 线程索引
  (checkpoint 已覆盖)。
