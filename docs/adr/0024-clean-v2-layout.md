...
- **Stage 4**：从 `v0.0.1` 移植并适配 prompt / skill / sandbox / memory(真实向量库 + 作为 RAG
  Retriever) / embeddings / scheduler(背景执行) / session(线程索引) / checkpoint 文件后端。
- **Stage 5**：补齐 demo（streaming-bus / resume / branch / deepagent / media）与 README、
  ARCHITECTURE 文档。
