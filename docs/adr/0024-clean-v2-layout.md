...
- **Stage 4(进行中)**:已从 `v0.0.1` 移植并适配:embeddings、sandbox + tool/exec、
  ...
  prompt builder(`WithPrompt`)。**待移植**:分层记忆系统(memx/textmem/workingmem/
  projectmem/rules,较重、绑 v1 session/prompt)、skill(依赖 sandbox+prompt+fs)、
  scheduler(背景执行,bus+Run 已可用 goroutine 替代)、session 线程索引(checkpoint 已覆盖)。
